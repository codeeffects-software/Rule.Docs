...
<small>**Introduced in Version:** [5.5.1.8](https://codeeffects.com/version/new-gettype-delegate-and-client-enhancements/5518)</small></em>
...
If assemblies that declare in-rule methods or rule actions are updated to newer or different versions, all references saved in [Rule XML](https://codeeffects.com/decision-automation/business-rules-storage) or [Source XML](https://codeeffects.com/decision-automation/business-rule-source-object-xml) become obsolete because they are stored as *assembly-qualified names*. In such cases, you need to update those references accordingly.

This delegate helps resolve type versioning issues. Implement your own type resolver method following the signature of the `GetTypeDelegate` delegate, and assign it to the `TypeGetter` property of the [`CodeEffects.Rule.Editor.Control`](https://codeeffects.com/decision-automation/rule-editor-control) editor's class to ensure that the editor loads your types according to your custom logic.
