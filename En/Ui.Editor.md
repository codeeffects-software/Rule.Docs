...
Rule Editor is the free web-based tool that helps rule authors to [create](https://codeeffects.com/decision-automation/business-rule-management) new business rules and load existing ones for modification, validation, or deletion. It consists of two components: the JavaScript class `codeeffects.editor.js`, which implements the editor’s client-side functionality, and the .NET assembly *CodeEffects.Rule.Editor.dll*, which implements its server-side logic. The [CodeEffects.Rule.Editor.Control.cs](https://codeeffects.com/decision-automation/rule-editor-control) class declared in that assembly defines the server-side functionality of the editor. See the [implementation article](https://codeeffects.com/decision-automation/business-rules-editor-implementation) for details on obtaining and implementing the editor in your project.
...
- <b>Toolbar</b> — An optional UI element at the top of the editor that helps manage rules. It’s included for convenience and is ideal for testing the Code Effects platform or for smaller production projects. You can use it as is or hide it and implement its functionality yourself. The Toolbar is enabled by default. Set `Control.ShowToolBar` to `false` to hide it. See its own [documentation topic](https://codeeffects.com/decision-automation/business-rule-toolbar) for details.

	![Collection item](https://codeeffects.com/resources/images/doc/EditorToolbar.jpg)

- <b>Help String</b> — An optional line of text between the Toolbar and the Rule Area that guides rule authors as they create or modify rules and warns about issues or errors. It’s enabled by default and uses the English culture. Set `Control.ShowHelpString` to `false` to hide it. Use the [Help XML](https://codeeffects.com/decision-automation/business-rule-multilingual-support) feature to control the culture and language of static UI labels and messages.

	![Collection item](https://codeeffects.com/resources/images/doc/EditorHelpString.jpg)

- <b>Rule Area</b> — The panel at the bottom of the editor that contains all elements of the rule being authored.

	![Collection item](https://codeeffects.com/resources/images/doc/EditorRuleArea.jpg)

Users can author rules using both [keyboard and mouse gestures](https://codeeffects.com/decision-automation/business-rule-editor-keyboard-gestures-support). [Cut, copy, and paste](https://codeeffects.com/decision-automation/business-rule-editor-cut-copy-paste-support) of rule elements are also supported.

Rule Editor can operate in [five modes](https://codeeffects.com/decision-automation/business-rule-editor-modes) that define the evaluation types of your rules. It’s important to understand what these modes are and how they work.
...
![Collection item](https://codeeffects.com/resources/images/doc/EditorBeginRule.jpg)
...
![Collection item](https://codeeffects.com/resources/images/doc/EditorParenthesesRule.jpg)
...
Rule Editor includes full support for .NET collections, including passing collections as parameters to in-rule methods and rule actions. If the source object declares a public collection, the Rule Area adds **Exists...** and **Does not exist...** options in field menus. Selecting one of these options switches the Rule Area into collection-authoring mode. See the [collections](https://codeeffects.com/decision-automation/business-rules-collection-data-types) data type topic for details on how to work with them.

## Numeric Calculations

In Code Effects, rules can declare numeric calculations involving [numeric](https://codeeffects.com/decision-automation/business-rules-data-types) fields and values. By default, all numeric fields can be compared to the result of a calculation unless this feature is explicitly disabled by setting `FieldAttribute.IncludeInCalculations` to `false`.
...
![Collection item](https://codeeffects.com/resources/images/doc/EditorCalculation.jpg)
...
![Collection item](https://codeeffects.com/resources/images/doc/EditorInvalidRule.jpg)