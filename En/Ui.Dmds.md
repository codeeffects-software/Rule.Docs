...
Dynamic Menu Data Sources are useful when you have a dynamic list of key/value pairs, and would like to use those values in [Rule Editor](https://codeeffects.com/decision-automation/business-rule-editor) as if they were members of a plain .NET enumerator. You can define one or many menu sources, and use them as field values, in-rule method return values, and even as parameter(s) of in-rule methods and rule actions.

A single menu source can be declared by any .NET class (including the [source object](https://codeeffects.com/decision-automation/business-rule-source-object)) as a parameterless static or public instance method that returns a collection of instances of the [DataSourceItem](https://codeeffects.com/decision-automation/rule-editor-client-datasourceitem) class. It can also be declared as a client parameterless function that returns an array of `{ ID: value, Name: value }` anonymous objects. Having a client function as the menu source is generally more advisable because any change to the underlying data will be available to the rule authors the moment they pull up the related menu. With server-side menu sources, the rule author needs to reload the entire page to get the latest changes, which is a non-issue if the data doesn't change very often.

<b>Note</b> that even though the method must be parameterless, there is a way to customize the returned collection of the dynamic source by using external values. Read more about it [in this topic](https://codeeffects.com/decision-automation/business-rules-dynamic-menu-data-sources-customized).
...
This simple [source object](https://codeeffects.com/decision-automation/business-rule-source-object) declares properties `FullName` and `RoleID`. It also declares a single rule action method called `Authorize` that takes `Status` enum as its single param. This source is also decorated with the CodeEffects.Rule.Common.Attributes.[Data](https://codeeffects.com/decision-automation/rule-common-attributes-data) attribute. It declares our menu source.
...
- Note the use of the `DataSourceName` property of the CodeEffects.Rule.Attributes.[Field](https://codeeffects.com/decision-automation/rule-common-attributes-field) attribute for the `RoleID` property. It tells Code Effects which menu source should be used for this property. The [Parameter](https://codeeffects.com/decision-automation/rule-common-attributes-parameter) and [Return](https://codeeffects.com/decision-automation/rule-common-attributes-return) attributes also declare the `DataSourceName` property, so you can use menu sources as parameters and return values of in-rule methods as well.
...
Let's see all of this in action. Assuming that we've already declared [Rule Editor](https://codeeffects.com/decision-automation/business-rule-editor) on a web page, and set our `User` class as its source object, we can load the page and see the data from the `GetRoles()` methods appear as the `RoleID` menu:

![Menu](https://codeeffects.com/resources/images/doc/datasourcesfield.jpg)

The rule displays names returned by the method...

![Menu](https://codeeffects.com/resources/images/doc/datasourcesresult.jpg)

... but the resulting [Rule XML](https://codeeffects.com/decision-automation/business-rules-storage) contains ID of the selected role instead of display name. This value will be used by the [Evaluator](https://codeeffects.com/decision-automation/rule-engine-evaluator-generic) during rule evaluation:

![Menu](https://codeeffects.com/resources/images/doc/datasourcesxml.jpg)
