...
Code Effects business rules engine comes with a powerful feature that allows you to reuse any [evaluation type](https://codeeffects.com/decision-automation/business-rule-evaluation-type) rule in any other rule as if it were a simple [field](https://codeeffects.com/decision-automation/rule-common-attributes-field) of the `System.Boolean` type.
...
If we were to register [Rule Editor](https://codeeffects.com/decision-automation/business-rule-editor) on a web page with this `Patient` class as a [source object](https://codeeffects.com/decision-automation/business-rule-source-object), we could create an evaluation type rule that checks whether the patient is `21` or older, name that rule <b>Legal Age</b>, and save it:

![Reusable Rule 1](https://codeeffects.com/resources/images/doc/reusablerule.jpg)

Later, we could reuse that rule in other rules as if it were a `Boolean` field called <em>Legal Age</em> by adding it to the collection of items in the <em>Rules</em> menu and the fields context menu the next time we load the page (more about adding items to the <em>Rules</em> menu can be found in the [Toolbar](https://codeeffects.com/decision-automation/business-rule-toolbar) topic). So, the selection of this rule from the <em>Rules</em> menu for editing would look like this:

![Reusable Rule 2](https://codeeffects.com/resources/images/doc/reusableedit.jpg)

It would also appear as an ordinary `Boolean` field in the context menu when you create other rules:

![Reusable Rule 3](https://codeeffects.com/resources/images/doc/reusablemenu.jpg)

Notice that the [Help String](https://codeeffects.com/decision-automation/business-rule-editor) displays the description of our rule every time the mouse hovers over its name in either menu.

Now we can reuse our <b>Legal Age</b> rule in any other rule that uses the same source object:

![Reusable Rule 4](https://codeeffects.com/resources/images/doc/reusableused.jpg)
...
Code Effects business rules editor does not allow rule authors to reuse a rule inside itself while that rule is being edited — it simply won't display the rule's name in the context menu even if the developer adds it to the menu's item collection. Rule Editor can also check for circular references during rule validation, but it does this only if it's able to retrieve the [Rule XML](https://codeeffects.com/decision-automation/business-rules-storage) of the referenced rule and examine it for inner references. You can force Rule Editor to perform such a check by supplying it with the [`GetRuleDelegate`](https://codeeffects.com/decision-automation/rule-common-models-getruledelegate) - a method that receives a rule ID and returns that rule's XML.
